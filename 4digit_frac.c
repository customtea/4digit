...
void pattern2(FRAC P_Num[],int R1,int R2,int R3);
void pattern3(FRAC P_Num[],int R1,int R2,int R3);
void pattern4(FRAC P_Num[],int R1,int R2,int R3);
	...
	for(R1=0;R1<4;R1++)
	{
		for(R2=0;R2<4;R2++)
		{
			for(R3=0;R3<4;R3++)
			{
				...
				pattern2(P_Num,R1,R2,R3);
				pattern3(P_Num,R1,R2,R3);
				pattern4(P_Num,R1,R2,R3);
			}
		}
	}
}
...
void pattern1(FRAC P_Num[],int R1,int R2,int R3)
{
	...
	if(t_R3.deno !=0 && t_R3.nume%t_R3.deno == 0)
	{
...
void pattern4(FRAC P_Num[],int R1,int R2,int R3)
{
	...
	FRAC t_R1,t_R2,t_R3;
	t_R1 = operant(R1,P_Num[0],P_Num[1]);
	t_R2 = operant(R2,P_Num[2],P_Num[3]);
	t_R3 = operant(R3,t_R1,t_R2);
